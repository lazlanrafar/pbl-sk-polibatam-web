      ...
      <div class="card-body">
        ...
        >
          ...
          <template v-slot:[`item.filepath`]="{ item }">
            <a :href="handleDownload(item.filepath)" :download="item.name"> Download Document </a>
          </template>
...
export default {
  ...
  computed: {
    reports() {
      return this.$store.state.suratTugas.reports;
    },
    isLoading() {
    ...
    optionsTable: {
      get() {
        return this.$store.state.suratTugas.optionsTable;
      },
      set(value) {
        this.$store.commit("SET_OPTIONS_TABLE_SURAT_TUGAS", value);
      },
    },
  },
  methods: {
