...
export default {
  ...
  computed: {
    ...
    optionsTableDokumenTugas: {
      get() {
        return this.$store.state.dokumenTugas.optionsTableDokumenTugas;
      },
      set(val) {
        this.$store.commit("SET_LIST_TAG_GROUP_DOKUMEN_TUGAS", val);
      },
    },
  },
  methods: {
    ...
    handleUpdate(id) {
      ...
      this.$store.dispatch("setFormDokumenTugas", id);
      this.$store.commit("SET_IS_UPDATE_DOKUMEN_TUGAS", id);
    },
    handleDelete(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store.dispatch("deleteDokumenTugas", id);
        }
      });
    },
    onFileChange(e) {
